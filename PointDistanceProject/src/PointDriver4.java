...
public class PointDriver4 {
	...
	public static void main(String[] args) throws IOException{
		...
		System.out.println("Remove duplicate entries? (Y/N)");
		
		String removedupes = keys.next();
		
		//Starts file in which to save points & distances

		File file2 = new File("src/pointsouttst.txt");
		...
		for(int c = 0; c<a1.getLength();c++){
			...
			write.write("Point: " + a1.getPoint(c).toString());
			write.newLine();
			write.flush();
			...
			if(removedupes.equals("Y") || removedupes.equals("y")){
				a1.removePoint(c);
			}
